...
export async function getCareerData(userId: string, token: string) {
  const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/career/${userId}`, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  })
  ...
  if (!response.ok) {
    const error = await response.json()
    throw new Error(error.message || "Error obteniendo datos de carrera")
  }
